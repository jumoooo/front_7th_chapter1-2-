...
- 사용자 경험을 최우선으로 고려
- 성능과 안정성을 동시에 고려
- 현재(2025-10-27) 이전의 작성된 코드들은 수정하지 않는다.
- 함수 및 테스트 코드 상단에 주석으로 'No Ai' 라는 글이 포함되어 있으면 해당 코드는 수정하지 않는다.
- 정적인 파일이나, Type 파일은 절대 수정하지 않는다.
...
- `mockdowns/agents/orchestrator.md` — 오케스트레이터: 전체 흐름 조율
- `mockdowns/agents/analyst.md` — Analyst: PRD, 수용 기준(AC) 도출
- `mockdowns/agents/pm.md` — PM: 우선순위, 릴리스 범위, 성공 지표
- `mockdowns/agents/architect.md` — Architect: 아키텍처, 경계, 계약

### 🔁 Development Cycle (Context-Engineered Development)

- `mockdowns/agents/scrum-master.md` — Scrum Master: Story files 운용
- `mockdowns/agents/dev.md` — Dev: TDD, Tidy First, 최소 구현
- `mockdowns/agents/qa.md` — QA: 수용 기준 검증, 사용자 중심 테스트

## 📁 작업물 관리 시스템

### 📋 산출물 저장 규칙

- **저장 위치**: 각 에이전트별 폴더 (`mockdowns/[에이전트명]/`)
